...
theorem sterbenz_exp_proximity (a b : FiniteFp) (ha : a.s = false) (hb : b.s = false)
  ...
  have ha_pos : (0 : R) < ⌞a⌟ := FiniteFp.toVal_pos a ha ha_nz
  ...
  · -- a.e - 1 ≤ b.e
    ...
    · have : (2 : R) ^ a.e < (2 : R) ^ (b.e + 2) :=
        calc (2 : R) ^ a.e ≤ ⌞a⌟ := FiniteFp.toVal_normal_lower a ha ha_normal
          _ ≤ 2 * ⌞b⌟ := h_ub
...
theorem sterbenz_aligned_diff_bound (a b : FiniteFp) (ha : a.s = false) (hb : b.s = false)
  ...
  have sig_le_of_half_le (x y : FiniteFp) (hxs : x.s = false) (hys : y.s = false)
      (hy_nz : 0 < y.m)
      (h_half : ⌞y⌟[R] / 2 ≤ ⌞x⌟[R]) (hye_eq : y.e = x.e + 1) :
      y.m ≤ x.m := by
    ...
    have key : (y.m : R) * (2 : R) ^ (y.e - prec + 1) ≤
      ...
      calc (y.m : R) * (2 : R) ^ (y.e - prec + 1)
          = ⌞y⌟ := (FiniteFp.toVal_pos_eq (R := R) y hys).symm
        _ ≤ 2 * ⌞x⌟ := by linarith
  ...
  · -- Case a.e > b.e (so e_min = b.e, a.e = b.e + 1)
    ...
    have h_lb' : ⌞a⌟[R] / 2 ≤ ⌞b⌟[R] := by linarith
...
theorem sterbenz_unique (a b : FiniteFp) (ha : a.s = false) (hb : b.s = false)
  ...
  obtain ⟨f, hf_eq, hf_val⟩ := sterbenz a b ha hb ha_nz hb_nz h_lb h_ub
  ...
  letI : RModeIdem R := RI'
  obtain ⟨g, hg_eq, hg_val⟩ := sterbenz a b ha hb ha_nz hb_nz h_lb h_ub
