...
section LegacyRoundIntSig

variable [FloatFormat]

private inductive RoundingMode
  | Down
  | Up
  | TowardZero
  | NearestTiesToEven
...
private def RoundingMode.toRoundingFunction {R : Type*}
    [Field R] [LinearOrder R] [IsStrictOrderedRing R] [FloorRing R]
    [FloatFormat] (mode : RoundingMode) : R → Fp :=
  match mode with
  | .Down => roundDown
  | .Up => roundUp
...
private def RoundingMode.round {R : Type*}
    [Field R] [LinearOrder R] [IsStrictOrderedRing R] [FloorRing R]
...
private def modeOfPolicyKind : RModePolicyKind → RoundingMode
  | .towardNeg => .Down
  | .towardPos => .Up
...
private theorem shouldRoundUp_modeOfPolicyKind (k : RModePolicyKind)
    ...
    shouldRoundUp (modeOfPolicyKind k) sign q r shift =
      policyShouldRoundUp k sign q r shift := by
  cases k <;> simp [shouldRoundUp, policyShouldRoundUp, modeOfPolicyKind]

private theorem handleOverflow_modeOfPolicyKind (k : RModePolicyKind)
    (sign : Bool) :
    handleOverflow (modeOfPolicyKind k) sign =
...
private theorem intSigVal_eq_global {R : Type*}
    ...
    intSigVal (R := R) sign mag e_base =
      _root_.intSigVal (R := R) sign mag e_base := by
  rfl

private theorem roundIntSigM_eq_roundIntSig_policy
    {R : Type*} [Field R] [LinearOrder R] [IsStrictOrderedRing R] [FloorRing R]
    [RMode R] [RModeExec] [RModePolicySpec R]
    (sign : Bool) (mag : ℕ) (e_base : ℤ) :
    _root_.roundIntSigM sign mag e_base =
      roundIntSig (modeOfPolicyKind (RModePolicyTag.kind (R := R))) sign mag e_base := by
  let k : RModePolicyKind := RModePolicyTag.kind (R := R)
  have hsr : ∀ (sgn : Bool) (q r sh : ℕ),
      RModeExec.shouldRoundUp sgn q r sh = policyShouldRoundUp k sgn q r sh := by
    intro sgn q r sh
    simpa [k] using (RModePolicySpec.shouldRoundUp_eq_policy (R := R) sgn q r sh)
  have hov : ∀ (sgn : Bool),
      RModeExec.handleOverflow sgn = policyHandleOverflow k sgn := by
    intro sgn
    simpa [k] using (RModePolicySpec.handleOverflow_eq_policy (R := R) sgn)
  unfold _root_.roundIntSigM roundIntSig
  simp_rw [hsr, hov]
  simp [k, shouldRoundUp_modeOfPolicyKind, handleOverflow_modeOfPolicyKind]

instance {R : Type*} [Field R] [LinearOrder R] [IsStrictOrderedRing R] [FloorRing R]
    [RMode R] [RModeExec] [RModePolicySpec R] : RoundIntSigMSound R where
  roundIntSigM_correct := by
    intro sign mag e_base hmag
    let k : RModePolicyKind := RModePolicyTag.kind (R := R)
    rw [roundIntSigM_eq_roundIntSig_policy (R := R) sign mag e_base]
    have hcore := roundIntSig_correct (R := R) (mode := modeOfPolicyKind k) sign mag e_base hmag
    rw [hcore]
    rw [round_modeOfPolicyKind (R := R) k]
    have hround := RModePolicySpec.round_eq_policy (R := R)
    have hisv : intSigVal (R := R) sign mag e_base = _root_.intSigVal (R := R) sign mag e_base := by
      simpa using intSigVal_eq_global (R := R) sign mag e_base
    rw [hisv]
    exact (hround (_root_.intSigVal (R := R) sign mag e_base)).symm

end LegacyRoundIntSig

end RoundIntSigPolicySound
